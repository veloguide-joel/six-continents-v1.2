...
// Handle password reset flow triggered by Supabase auth state events
async function handlePasswordResetFromAuthState(session) {
    const email = session?.user?.email || '<unknown>';
    console.log('[PasswordReset] PASSWORD_RECOVERY event detected for', email);

    try {
        const newPassword = prompt('Enter your new password (min 6 characters):');
        if (!newPassword || newPassword.length < 6) {
            console.log('[PasswordReset] User cancelled or provided invalid password');
            return;
        }
        ...
        try { history.replaceState({}, document.title, window.location.pathname + window.location.search); } catch (e) { /* noop */ }

        try { alert('Password updated. Continuing into the app...'); } catch (e) { /* noop */ }

        // Call the shared post-login flow so the user is taken into the game
        try {
            if (session?.user && typeof onUserSignedIn === 'function') {
                await onUserSignedIn(session.user);
            } else {
                // As a fallback, attempt to fetch current user and call onUserSignedIn
                try {
                    const { data: userData, error: userErr } = await supabase.auth.getUser();
                    if (!userErr && userData?.user && typeof onUserSignedIn === 'function') {
                        await onUserSignedIn(userData.user);
                    }
                } catch (e) {
                    console.warn('[PasswordReset] Fallback getUser/onUserSignedIn failed:', e);
                }
            }
        } catch (e) {
            console.warn('[PasswordReset] onUserSignedIn failed after password update:', e);
        }

    } catch (err) {
        console.error('[PasswordReset] unexpected error handling reset:', err);
    }
}
...
function initializeSupabase() {
    if (window.supabase) {
        ...
        supabase.auth.onAuthStateChange(async (event, session) => {
            ...
            // Handle Supabase password recovery flow by delegating to the shared helper
            if (event === 'PASSWORD_RECOVERY') {
                try {
                    await handlePasswordResetFromAuthState(session);
                } catch (err) {
                    console.warn('[PasswordReset] handlePasswordResetFromAuthState failed:', err);
                }
            ...
            if (session?.user) {
                ...
                if (session.user.email === ADMIN_EMAIL) {
                    ...
                    try {
                        ...
                        console.warn('[AUTH] onUserSignedIn failed (non-fatal):', err);
                        // fallback: attempt to show game UI
